...
function PlumAccentInner({ state, scale = 1 }: Props) {
  ...
  const mainHead: React.CSSProperties = {
    ...
    borderBottom: `1px solid ${hexWithAlpha(accent, 0.25)}`, paddingBottom: '5px',
  ...
  const sideHead: React.CSSProperties = {
    ...
    textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(255,255,255,0.6)',
  ...
  return (
    <div style={{ transform: `scale(${scale})`, transformOrigin: 'top left', width: scale !== 1 ? `${100 / scale}%` : undefined }}>
      <div id="plum-accent-template" data-photo-enabled="true" style={{ fontFamily: fonts.body, display: 'flex', minHeight: '297mm', width: '210mm', boxSizing: 'border-box' }}>

        {/* Rich plum/accent sidebar 34% */}
        <div style={{ width: '34%', background: accent, padding: `${Math.round(30 * sp)}px ${Math.round(16 * sp)}px`, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {/* Circular photo */}
          {p?.avatarUrl ? (
            <img src={p.avatarUrl} alt="Profile" style={{ width: '88px', height: '88px', borderRadius: '50%', objectFit: 'cover', border: '3px solid rgba(255,255,255,0.4)', boxShadow: '0 4px 14px rgba(0,0,0,0.25)', marginBottom: `${Math.round(14 * sp)}px` }} />
          ) : (
            <div style={{ width: '88px', height: '88px', borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24pt', fontWeight: '700', color: 'rgba(255,255,255,0.8)', fontFamily: fonts.heading, border: '3px solid rgba(255,255,255,0.3)', marginBottom: `${Math.round(14 * sp)}px` }}>
              {p?.firstName?.[0] ?? '?'}{p?.lastName?.[0] ?? ''}
            </div>
          )}
          <p style={{ fontFamily: fonts.heading, fontSize: '12.5pt', fontWeight: '700', color: '#fff', margin: '0 0 3px', textAlign: 'center', lineHeight: '1.2' }}>
            {p ? `${p.firstName} ${p.lastName}` : 'Your Name'}
          </p>
          {p?.professionalTitle && (
            <p style={{ fontSize: '8pt', color: 'rgba(255,255,255,0.75)', margin: '0 0 14px', textAlign: 'center', fontStyle: 'italic' }}>{p.professionalTitle}</p>
          )}

          {/* Decorative divider */}
          <div style={{ width: '32px', height: '2px', background: 'rgba(255,255,255,0.35)', borderRadius: '1px', marginBottom: `${Math.round(10 * sp)}px` }} />

          {/* Contact */}
          <div style={{ ...sideHead, marginTop: '0' }}>Contact</div>
          <div style={{ fontSize: '7.5pt', color: 'rgba(255,255,255,0.8)', lineHeight: '1.9', textAlign: 'center', wordBreak: 'break-all', width: '100%' }}>
            ...
            <>
              ...
              <div style={{ width: '100%' }}>
                {profile.skills.slice(0, 9).map((sk) => {
                  ...
                  return (
                    <div key={sk.id} style={{ marginBottom: '7px' }}>
                      <span style={{ fontSize: '7.5pt', color: 'rgba(255,255,255,0.85)' }}>{sk.name}</span>
                      <div style={{ height: '4px', background: 'rgba(255,255,255,0.15)', borderRadius: '2px', marginTop: '2px' }}>
                        <div style={{ height: '4px', width: `${pct}%`, background: 'rgba(255,255,255,0.75)', borderRadius: '2px' }} />
                      </div>
                    </div>
                  )
                })}
              ...
              <div style={{ width: '100%' }}>
                {profile.languages.map((lang) => (
                  <div key={lang.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '7pt', color: 'rgba(255,255,255,0.5)' }}>{lang.level}</span>
                  </div>
                ))}
              </div>
            </>
        ...
        {/* Main white 66% */}
        <div style={{ flex: 1, background: '#fff', padding: `${Math.round(30 * sp)}px ${Math.round(22 * sp)}px`, boxSizing: 'border-box' }}>
...
export const PlumAccentTemplate = React.memo(PlumAccentInner)
